...
def parse_enemy_frontmatter(content):
    ...
    while i < len(lines):
        ...
        # Intent list item (2 spaces, starts with - id:)
        if current_section == 'intents' and indent == 2 and stripped.startswith('- id:'):
            if current_intent:
                intents.append(current_intent)
            current_intent = {'id': stripped[5:].strip()}
            i += 1
            continue

        # Intent property (4 spaces)
        if current_section == 'intents' and current_intent and indent >= 4:
            if ':' in stripped:
                key, _, value = stripped.partition(':')
                key = key.strip()
                value = value.strip().strip('"').strip("'")

                # Handle nested skill object
                if key == 'skill' and value == '':
                    current_intent['skill'] = {}
                    i += 1
                    # Read nested skill properties
                    while i < len(lines):
                        nested_line = lines[i]
                        nested_stripped = nested_line.strip()
                        nested_indent = len(nested_line) - len(nested_line.lstrip())
...
def parse_summon_file(filepath):
    """Parse a summon .md file using the same format as enemies."""
    content = filepath.read_text(encoding='utf-8')
    frontmatter, body = parse_enemy_frontmatter(content)  # Reuse enemy parser

    # The body is the summon description (optional flavor text)
    description = body.strip()
    if description:
        frontmatter['description'] = description
...
def generate_summons_js(summons):
    """Generate the JavaScript summons file content."""
    # Build the summons object
    summons_dict = {}
    for summon in summons:
        summons_dict[summon['id']] = summon
    ...
    lines = [
        ...
        ' */',
        '',
        'var summonDefinitions = ' + json.dumps(summons_dict, indent=2) + ';',
        '',
...
def main():
    ...
    # === Process Summons ===
    print("\n" + "=" * 40)
    print("Processing summons...")

    summons = []
    if SUMMONS_DIR.exists():
        summon_files = list(SUMMONS_DIR.glob('*.md'))
        if summon_files:
            print(f"Found {len(summon_files)} summon files")

            for filepath in summon_files:
                print(f"  Parsing: {filepath.name}")
                try:
                    summon = parse_summon_file(filepath)
                    if 'id' not in summon:
                        print(f"    Warning: No 'id' field, using filename: {filepath.stem}")
                        summon['id'] = filepath.stem
                    summons.append(summon)
                except Exception as e:
                    print(f"    Error: {e}")
                    sys.exit(1)

            # Check for duplicate summon IDs
            summon_ids = [s['id'] for s in summons]
            summon_duplicates = [id for id in summon_ids if summon_ids.count(id) > 1]
            if summon_duplicates:
                print(f"\nError: Duplicate summon IDs found: {set(summon_duplicates)}")
                sys.exit(1)

            # Generate summons.js
            print(f"\nGenerating {SUMMONS_OUTPUT_FILE}...")
            summons_content = generate_summons_js(summons)

            with open(SUMMONS_OUTPUT_FILE, 'w', encoding='utf-8') as f:
                f.write(summons_content)
        else:
            print("  No summon files found (optional)")
    else:
        print(f"  No summons/ directory found (optional)")
    ...
    if summons:
        print(f"  Summons: {len(summons)} summons")
    if player_data:
    ...
    if summons:
        print(f"  Summons: {SUMMONS_OUTPUT_FILE}")
    if player_data:
